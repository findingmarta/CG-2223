...
void torus(float inner_radius, float outer_radius, int slices, int stacks, string file) {
	...
	for (int i = 0; i < slices; i++) {
		...
		for (int j = 0; j < stacks; j++) {
			...
			y = inner_radius * sin(phi);
			z = (outer_radius + inner_radius * cos(phi)) * sin(theta);
			t1 = (float)i / slices;
			write << x << " " << y << " " << z << " " << nx << " " << ny << " " << nz << " " << t1 << " " << t2 << endl;

			x = (outer_radius + inner_radius * cos(phi)) * cos(theta + delta_slices);
			y = inner_radius * sin(phi);
			z = (outer_radius + inner_radius * cos(phi)) * sin(theta + delta_slices);
			t1 = (float)(i + 1) / slices;
			write << x << " " << y << " " << z << " " << nx << " " << ny << " " << nz << " " << t1 << " " << t2 << endl;

			x = (outer_radius + inner_radius * cos(phi + delta_stacks)) * cos(theta + delta_slices);
			y = inner_radius * sin(phi + delta_stacks);
			z = (outer_radius + inner_radius * cos(phi + delta_stacks)) * sin(theta + delta_slices);
			t1 = (float)(i + 1) / slices;
			write << x << " " << y << " " << z << " " << nx << " " << ny << " " << nz << " " << t1 << " " << t2 << endl;
			...
			y = inner_radius * sin(phi + delta_stacks);
			z = (outer_radius + inner_radius * cos(phi + delta_stacks)) * sin(theta);
			t1 = (float)i / slices;
			write << x << " " << y << " " << z << " " << nx << " " << ny << " " << nz << " " << t1 << " " << t2 << endl;
